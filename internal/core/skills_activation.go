...
const (
	exactSkillNameScore       = 100
	allSkillNameTokensScore   = 80
	minimumActivationMatches  = 2
	activationPhraseBaseScore = 40
)
...
func skillNameActivationScore(
	...
	if containsSkillPhrase(promptLower, nameLower) {
		return exactSkillNameScore, "exact skill name mention"
	}
	...
	return allSkillNameTokensScore, "all skill name tokens matched"
}
...
func skillDescriptionActivationScore(promptTokens map[string]bool, description string) (score int, reason string) {
	...
	for _, phrase := range skillActivationPhrases(description) {
		...
		if matches < minimumActivationMatches {
			continue
		}

		score := activationPhraseBaseScore + matches
		if score > bestScore {
			bestScore = score
