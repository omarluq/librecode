...
func (app *App) watchToolTaskCompletions(ctx context.Context) {
	...
	if err != nil {
		app.logToolTaskRefreshError(ctx, err)
...
func (app *App) hasRunningToolTasks() bool {
	for index := range app.toolTasks {
		...
		case database.TaskQueued, database.TaskRunning:
			return true
		case database.TaskCanceling, database.TaskSucceeded, database.TaskFailed,
			database.TaskCanceled, database.TaskInterrupted:
		}
	}
