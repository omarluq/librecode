...
func TestActiveWorkflowAppearsInAgentSummary(t *testing.T) {
	...
	assert.Contains(t, lines[2].Text, "STATUS")
	assert.Contains(t, lines[3].Text, "review[0]")
	assert.Contains(t, lines[3].Text, string(database.TaskRunning))
